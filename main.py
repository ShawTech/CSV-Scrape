...
w = open('dummy.csv','w',encoding='utf-8')
...
#         print(accident_time)
    ...
    #         print(accident_speed[0])
            writer.writerow({'Latitude': table[21][count], 'Longitude': table[20][count], 'Accident_Date': table[6][count], 'Accident_Time': round((random.uniform(0.01, 23.59)*60)/(24*60),4), 'Light_Condition': random.choice([0, 0.5, 1]), 'Speed_Zone': round(random.uniform(0, 120)/120, 4), 'Crash_OR_NOT': 0})
        count += 1
except ValueError:
